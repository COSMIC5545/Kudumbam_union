...
export const StatusStrip: React.FC<StatusStripProps> = ({
  ...
  const getJudgementLevel = () => {
    if (chatStatus === 'verdict_ready') return { text: 'MAXIMUM 🔥', color: 'text-red-400' };
    if (chatStatus === 'analyzing') return { text: 'Analyzing...', color: 'text-ku-goldLight' };
  ...
  const getGossipPotential = () => {
    if (chatStatus === 'analyzing') return { text: 'Examining 🔍', color: 'text-ku-tealLight' };
    if (chatStatus === 'verdict_ready') return { text: 'Off the charts', color: 'text-ku-tealLight' };
